...
First command results in a numeric vector. The second results in a character vector. The third command results in character vector, because it includes character values.
...
Sep control how the terms are pasted together, while collapse controls how the results are pasted together. When collapse is NULL, we might get vector of character string but when specifying a value for collapse, you will get a single character string.
...
we use `read.table` and set `sep='\t',` or we could use the default parameters for `read.delim`.
