...
The subset of `gapminder` contains `r nrow(gap_africa)` rows. This is corresponds to `r length(unique(gap_africa$country))` countries over `r length(unique(gap_africa$year))` years.
...
```{r}
avg_lifeExp <- group_by(gapminder, country) %>%
  summarise(mean_lifeExp = mean(lifeExp)) %>%
  ungroup()

```

The country with longest average life expectancy is `r arrange(avg_lifeExp, desc(mean_lifeExp))$country[1]`, and the country with the shortest average life expectancy is `r arrange(avg_lifeExp, mean_lifeExp)$country[1]` .
