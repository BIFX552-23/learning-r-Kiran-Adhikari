...
This return the first items of cats, which is the coat variables. When using single square brackets `([)` this will return a data.frame with single variables or column of data. In this case a character vector, coat.

```{r}
cats[[1]]
```

This also gives us first item if cat data frame, but instead of a data frame, the use of double square brackets `([[)` returns a character vector.

```{r}
cats$coat
```

Using dollar sign `($)` results in the same character vector as if we used a double square brackets.
...
This return the value 'calico' which is the first observation of the first column of data in the data.frame.

```{r}
cats[, 1]
```

This return a character vector of the first column of data, including all observation of rows.

```{r}
cats[1, ]
```

This return a data.frame containing all variables for the first observation.
...
`length(matrix_example)` return 18, because there are a total 18 values in matrix (3rows and 6 columns).
