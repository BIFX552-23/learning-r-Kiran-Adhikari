...
```{r}
# create people
people <-data.frame(first_name = 'Kiran',
                    last_name = 'Adhikari',
                    lucky_number = 7)
  
# add a team member
people <- rbind(people, data.frame(first_name = 'Donald',
                    last_name = 'Duck',
                    lucky_number = pi) )
...
```{r}
source('script/load_gapminder.R')
```
...
Explain the output of `str(gapminder)` : Firstly, we are working with a tibble data frame. We have 1704 row and 6 columns in data frame. We have 6 variables.

-   Country factor with 142 levels

-   continents is a factor with 5 levels

-   year is an integer vector with length 1704.

-   lifeExp is a numeric vector with l1704 data length.

-   pop is an integer vector of length 1704

-   gdpPercap is numeric vector with 1704 length.
